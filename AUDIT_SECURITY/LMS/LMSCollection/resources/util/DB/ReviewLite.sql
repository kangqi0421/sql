...
SET PAUSE OFF
SET TERMOUT OFF
REM     ORACLE - License Management Services - Review Lite Script
REM
REM     Usage:
REM
...
REM     ----------- ------- ---------------
REM     2005-03-04  1.0     -
REM     2005-08-07  2.0     anherrer
REM     2005-10-03  2.1     fllanos
REM     2006-10-09  2.2     esiytiu
REM     2007-07-24  2.3.1   esiytiu
REM     2007-12-28  2.4.1   esiytiu
REM     2008-02-23  2.5     sserban
REM     2008-04-22  2.5.1   sserban
REM     2008-11-11  2.5.2   sserban
REM     2009-02-06  2.5.3   beparu
REM     2009-03-12  2.5.4   sserban
REM     2009-07-24  2.5.5   beparu
REM     2010-01-15  2.5.6   beparu
REM     2010-04-13  2.5.7   beparu
REM     2010-09-14  2.5.8   adimitrescu
REM     2011-03-14  2.5.9   sserban
REM     2011-08-02  2.6     adimitrescu
REM     2011-09-28  2.6.1   adimitrescu
REM     2011-11-04  12.3    ajgonzal
REM     2013-06-01  14.1    sserban
REM     2013-08-20  14.1.1  sserban
REM     2013-09-30  14.1.2  sserban
REM     2013-11-20  14.2.0  sserban
REM     2014-02-12  14.2.1  sserban
REM     2014-08-31  15.1    sserban
REM     2015-10-21  16.2    sserban
REM     2016-10-17  17.1    sserban
REM     2016-12-12  17.1.3  sserban
REM     2017-01-30  17.1.5  sserban
REM     2017-03-20  17.2    sserban
REM
...
--#<LMSCT>#define SCRIPT_LA=''                   -- script prompts for license agreement [default behavior]
-- Set SCRIPT_SI to run in interactive or in silent mode
--#<LMSCT>#define SCRIPT_SI=_SI_IGNORE_THIS_ERR  -- script does not prompt for privilege check confirmation
--#<LMSCT>#define SCRIPT_SI=''                   -- script prompts for privilege check confirmation [default behavior]
...
--#<LMSCT>#HOST&SCRIPT_LA more lms_license_agreement.txt   2> fii_err.txt
...
--#<LMSCT>#HOST&SCRIPT_LA rm   lms_license_agreement.txt   2> fii_err.txt
--#<LMSCT>#HOST&SCRIPT_LA del  lms_license_agreement.txt   2> fii_err.txt
...
BEGIN
  if    '&SEL_VER' = 'UNKNOWN' then
    dbms_output.put_line('DB: LMS-02014: ERROR: [&&INSTANCE_NAME]: Current database user ' || USER || ' does not have the privilege to query V$INSTANCE. Cannot continue.');
    dbms_output.put_line('Exiting...');
    :MIS_PRIV := 'X';
    return;
  elsif '&SEL_PRIV' = 'N' then
    dbms_output.put_line('DB: LMS-02802: WARNING: [&&INSTANCE_NAME]: Cannot check privileges. Continuing anyway. Errors expected.');
    dbms_output.put_line('.');
    :MIS_PRIV := 'N';
    return;
  elsif '&SEL_VER' = '8' and '&SEL_O7' = 'UNKNOWN' then
    dbms_output.put_line('DB: LMS-02803: WARNING: [&&INSTANCE_NAME]:  Current database user ' || USER || ' does NOT have the privilege to query V$PARAMETER. Continuing anyway. Errors expected.');
    SEL_DICT_REQUIRED := 'N';
  elsif '&SEL_VER' = '8' and '&SEL_O7' = 'FALSE' then
    dbms_output.put_line('DB: LMS-02804: WARNING: [&&INSTANCE_NAME]: O7_DICTIONARY_ACCESSIBILITY=FALSE. Access to the data dictionary views requires explicit object privilege. Errors expected.');
    SEL_DICT_REQUIRED := 'N';
  ...
  loop
    ...
    begin
      ...
      if    MIS_AUTH_TEXT is not null then
        MIS_AUTH_TEXT := 'DB: LMS-02016: ERROR: [&&INSTANCE_NAME]: Current database user ' || USER || ' does NOT have authorization on ' || MIS_AUTH_TEXT;
        MIS_AUTH := 'Y';
       :MIS_PRIV := 'Y';
    ...
    exception
      when others then
        ERR_C := SQLCODE;
        if     ERR_C = -01031 then
          MIS_AUTH_TEXT := 'DB: LMS-02017: ERROR: [&&INSTANCE_NAME]: Current database user ' || USER || ' does NOT have authorization on Oracle Database Vault realm';
          MIS_AUTH := 'Y';
         :MIS_PRIV := 'Y';
        else
  ...
  if   :MIS_PRIV = 'Y' and MISSING_PRIVS is not null then
    dbms_output.put_line('DB: LMS-02015: ERROR: [&&INSTANCE_NAME]: Current database user ' || USER || ' does NOT have ' || MISSING_PRIVS);
  end if;
  if MIS_AUTH = 'Y' then
  ...
  if :MIS_PRIV = 'Y' or MIS_AUTH = 'Y' then
    ...
    dbms_output.put_line('!        and information collected from this environment. Due to this, Oracle LMS may have to get back to you');
    dbms_output.put_line('!        and ask for additional items, or to execute again.');
    dbms_output.put_line('!');
  end if;

  if    '&SCRIPT_SI' is not null and :MIS_PRIV = 'Y' then
    dbms_output.put_line('DB: LMS-02807: WARNING: [&&INSTANCE_NAME]:  Running in silent mode. Continuing with missing privileges. Errors expected.');
  end if;
...
PROMPT
PROMPT LMS ReviewLite &SCRIPT_RELEASE. output file created at &SYSDATE_START
PROMPT
PROMPT
PROMPT DB CPU COUNT
PROMPT ==========================================================
...
prompt If rows are returned, then TUNING PACK or REAL APPLICATION TESTING license is needed
