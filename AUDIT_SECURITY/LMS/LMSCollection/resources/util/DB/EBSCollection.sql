...
--#<LMSCT>#--{
--#<LMSCT>#--Detect SQL*Plus client path separator
--#<LMSCT>#--Using some Unix/Linux specific syntax
--#<LMSCT>#host echo select \'$PWD\' as PWD_, \'rm\' as RMDEL_, \'/\' as PSEP_ from dual where \'$PWD\' like \'%/%\'\; > psep.sql 2> fii_err.txt
--#<LMSCT>#
--#<LMSCT>#define PWD=*
--#<LMSCT>#define RMDEL=del
--#<LMSCT>#define PSEP=\
--#<LMSCT>#col PWD_   new_val PWD   noprint
--#<LMSCT>#col RMDEL_ new_val RMDEL noprint
--#<LMSCT>#col PSEP_  new_val PSEP  noprint
--#<LMSCT>#-- The query syntax is correct only on Unix/Linux
--#<LMSCT>#SET TERMOUT OFF
--#<LMSCT>#@psep.sql
--#<LMSCT>#SET TERMOUT ON
--#<LMSCT>#-- Cleanup
--#<LMSCT>#host &RMDEL psep.sql   2> fii_err.txt
--#<LMSCT>#--}
...
BEGIN
  ...
  dbms_output.put_line('EBS: LMS-06000: COLLECTED: EBS Repository: ' || INSTANCE_N || PDB_N || ' running on Host: ' || HOST_N);
END;
/
SPOOL OFF
--#<LMSCT># }
