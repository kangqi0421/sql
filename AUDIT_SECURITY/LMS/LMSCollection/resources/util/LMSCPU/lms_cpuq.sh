...
setOutputFiles() {
	...
	ORA_MACHINFO_FILE="$TMPDIR"/${MACHINE_NAME}-lms_cpuq.txt 
...
setOSSystemInfo() {
	...
	if [ "$SCRIPT_USER" = "ROOT" ] ; then
			...
			if [ $? -eq 0 ] ; then
				$ECHO "LMSCPU: LMS-01104: WARNING: VMWare virtual machine"
			fi
			
			/usr/sbin/dmidecode 2>/dev/null | egrep -i 'virtualbox' >/dev/null 2>&1
			if [ $? -eq 0 ] ; then
				$ECHO "LMSCPU: LMS-01104: WARNING: Oracle VirtualBox machine"
			fi	
		else
			$ECHO "LMSCPU: LMS-01101: WARNING: /usr/sbin/dmidecode command not executed - $SCRIPT_USER insufficient privileges"
		fi
		...
		if [ $? -eq 0 ] ; then
			...
			if [ $SUNCHECK -ne 0 ] ; then
				$ECHO "LMSCPU: LMS-01102: WARNING: Possible Virtual Machine $SYSCON, processor information is also needed for the physical machine"
			fi
		fi
		...
		if [ ${MINOR} -gt 9 ] ; then
			...
			if [ "$ZONENAME" != "global" ] ; then
				$ECHO "LMSCPU: LMS-01103: WARNING: ${0} executed in the $ZONENAME zone, processor information is also needed for the global zone"
			fi
			set -xv
...
If a Master Agreement does not apply then the License Agreement displayed below applies to your use of the Software.
...
PLEASE SCROLL DOWN AND READ ALL OF THE FOLLOWING TERMS AND CONDITIONS OF THIS LICENSE AGREEMENT (\"Agreement\") CAREFULLY.  THIS AGREEMENT IS A LEGALLY BINDING CONTRACT BETWEEN YOU AND ORACLE AMERICA, INC. THAT SETS FORTH THE TERMS AND CONDITIONS THAT GOVERN YOUR USE OF THE SOFTWARE. 

YOU MUST ACCEPT AND ABIDE BY THESE TERMS AND CONDITIONS AS PRESENTED TO YOU - ANY CHANGES, ADDITIONS OR DELETIONS BY YOU TO THESE TERMS AND CONDITIONS WILL NOT BE ACCEPTED BY US AND WILL NOT MAKE PART OF THIS AGREEMENT.

Definitions
\"We,\" \"Us,\" and \"Our\" refers to Oracle America, Inc.  \"Oracle\" refers to Oracle Corporation and its affiliates.  

\"You\" and \"Your\" refers to the individual or entity that wishes to use the Software (as defined below) provided by Oracle. 

\"Software\" refers to the tool(s), script(s) and/or software product(s) and any applicable documentation provided to You by Oracle which You wish to access and use to measure, monitor and/or manage Your usage of separately-licensed Oracle software. 
...
-	make the Software available in any manner to any third party for use in the third party's business operations, without Our prior written consent;
...
-	cause or permit reverse engineering (unless required by law for interoperability), disassembly or decompilation of the Software (the foregoing prohibition includes but is not limited to review of data structures or similar materials produced by Software);
-	disclose results of any Software benchmark tests without Our prior written consent; 
...
End of Agreement
You may terminate this Agreement by destroying all copies of the Software. We have the right to terminate Your right to use the Software at any time upon notice to You, in which case You shall destroy all copies of the Software. 
...
Contact Information
Should You have any questions concerning Your use of the Software or this Agreement, please contact Oracle License Management Services at: http://www.oracle.com/us/corporate/license-management-services/index.html
...
printMachineInfo() {
	...
	$ECHO "LMS_CT Version=${LMSCT_BUILD_VERSION}"
...
# see if any check* Oracle LMS scripts are running, if not then print license. 
if [ "${LMSCT_BUILD_VERSION}" = "" ] ;then
	STANDALONE=true
	beginMsg
	CredentialValidation
fi
