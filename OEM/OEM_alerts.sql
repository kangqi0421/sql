...
select
   --  i.issue_id,
   decode(i.severity, 0, 'Clear', 4, 'Advisory', 8, 'Warning', 16, 'Critical', 32, 'Fatal') state,
...
--       i.issue_type,
--       i.creation_date,
   CAST((FROM_TZ(CAST(i.creation_date AS TIMESTAMP),'GMT') AT TIME ZONE 'EUROPE/PRAGUE') AS DATE) "creation_date",
--       i.owner,
...
-- issue id
select * from   sysman.em_issues_msg where issue_id in ('606E6D3EC3D453E0E053B011B10ADAED');
select * from sysman.em_issues_internal where issue_id in ('606E6D3EC3D453E0E053B011B10ADAED');
