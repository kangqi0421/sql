...
case when e.time_waited >= nvl(b.time_waited,0)
then e.time_waited - nvl(b.time_waited,0)
else e.time_waited end time,
e.time_waited total_time
...
(SNAP_ID, EVENT, TOTAL_WAITS, TOTAL_TIMEOUTS, TIME_WAITED, 
 C_TOTAL_WAITS, C_TOTAL_TIMEOUTS, C_TIME_WAITED)
AS 
select
SNAP_ID,EVENT,TOTAL_WAITS,TOTAL_TIMEOUTS,TIME_WAITED,C_TOTAL_WAITS,C_TOTAL_TIMEOUTS,C_TIME_WAITED
from
(
select
 SNAP_ID,EVENT,TOTAL_WAITS,TOTAL_TIMEOUTS,TIME_WAITED,C_TOTAL_WAITS,C_TOTAL_TIMEOUTS,C_TIME_WAITED,
 ROW_NUMBER() OVER( PARTITION BY SNAP_ID ORDER BY C_TIME_WAITED  desc) as RN
from
(
select
 SNAP_ID,
...
ORDER BY SNAP_ID ),TOTAL_TIMEOUTS)  as C_TOTAL_TIMEOUTS,
 TIME_WAITED-nvl(LAG(TIME_WAITED,1) OVER ( PARTITION BY EVENT ORDER BY
SNAP_ID ),TIME_WAITED) as C_TIME_WAITED
from STATS$SYSTEM_EVENT
where EVENT not in ( select EVENT from STATS$IDLE_EVENT )
)
)
where RN<6 and C_TIME_WAITED > 0;
/
