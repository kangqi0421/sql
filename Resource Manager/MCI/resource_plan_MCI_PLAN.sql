...
--  - MCI_APP - aplikaèní servery pøes vytvoøené db services
--  - MCI_HIGH - SQL ad-hoc query personální úèty
--
-- MCI_HIGH - if query consumes more than 5 seconds of CPU
--  - switch to the group GRP_LOW
--  - limit CPU time
...
DEFINE DEFAULT_SVC=MCIPK
DEFINE ADHOC_SVC=MCI
DEFINE CICDTP_SVC=CIC_DTP
DEFINE CIC_SVC=CIC
DEFINE INETEXT_SVC=INETEXT
DEFINE INETINT_SVC=INETINT
DEFINE GENESYS_SVC=GENESYS
DEFINE JOBS_SVC=MCI_JOBS
...
dbms_resource_manager.create_plan_directive(
		...
		, switch_time => 5
    , switch_for_call => FALSE    -- po dokonceni sql se uz nevracej zpet do HIGH groupy
    , switch_estimate => FALSE    -- dle cpu time, nikoliv dle exec planu
);
...
-- TOAD, SQL developer atd. spadne taktez do GRP_HIGH
dbms_resource_manager.set_consumer_group_mapping(
  dbms_resource_manager.client_program,'SQL Developer','&GRP_HIGH'
  );  
dbms_resource_manager.set_consumer_group_mapping(
  dbms_resource_manager.client_program,'plsqldev.exe','&GRP_HIGH'
  );
dbms_resource_manager.set_consumer_group_mapping(
  dbms_resource_manager.client_program,'Toad.exe','&GRP_HIGH'
  );  

-- map DBSNMP to SYS groups  
dbms_resource_manager.set_consumer_group_mapping(
  dbms_resource_manager.oracle_user,'DBSNMP','SYS_GROUP'
  );  
...
dbms_resource_manager.set_consumer_group_mapping(
	dbms_resource_manager.service_name, '&GENESYS_SVC', '&GRP_APP'
  );
dbms_resource_manager.set_consumer_group_mapping(
